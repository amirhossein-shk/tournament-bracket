export function tournamentBracket(userConfig = {}) {
    ...
    function validateRoundsShape(rounds) {
        ...
        rounds.forEach((round, roundIndex) => {
            if (!round || typeof round !== "object") {
                throw new Error(`Round at index ${roundIndex} must be an object.`);
            }

            if (!Array.isArray(round.matches)) {
                throw new Error(`rounds[${roundIndex}].matches must be an array.`);
            }

            if (round.matches.length === 0) {
                throw new Error(`rounds[${roundIndex}].matches must not be empty.`);
            }

            round.matches.forEach((match, matchIndex) => {
                if (!match || typeof match !== "object") {
                    throw new Error(
                        `rounds[${roundIndex}].matches[${matchIndex}] must be an object.`
                    );
                }

                if (!Array.isArray(match.players)) {
                    throw new Error(
                        `rounds[${roundIndex}].matches[${matchIndex}].players must be an array.`
                    );
                }

                if (match.players.length !== 2) {
                    throw new Error(
                        `rounds[${roundIndex}].matches[${matchIndex}] must have exactly 2 players.`
                    );
                }
            });
        });
        ...
        if (!isPowerOfTwo(firstRoundMatchesCount)) {
            throw new Error("First round matches count must be a power of two.");
        }
        ...
        for (let i = 1; i < rounds.length; i++) {
            ...
            if (actualMatchesCount !== expectedMatchesCount) {
                throw new Error(
                    `Invalid bracket shape: rounds[${i}] must have ${expectedMatchesCount} matches, but got ${actualMatchesCount}.`
                );
            }
        }
        ...
        if (lastRound.matches.length !== 1) {
            throw new Error("Last round must have exactly 1 match.");
        }
    }
    ...
    function validateConfig() {
        ...
        if (!container) {
            throw new Error(`Element with id "${config.targetId}" not found`);
        }
    ...
    function normalizeMatch(match) {
        ...
        return {
            ...
            players: [
                { ...DEFAULT_PLAYER, ...match.players?.[0] },
                { ...DEFAULT_PLAYER, ...match.players?.[1] },
            ],
        };
    }
    ...
    function createEmptyRound(matchesCount, roundIndex) {
        return {
            roundId: createId("round"),
            name: `Round ${roundIndex + 1}`,
            matches: Array.from({ length: matchesCount }, createEmptyMatch),
        };
    }
    ...
    function isPendingScore(score) {
        return score === "-" || score === "" || score === null || score === undefined;
    }
    ...
    function getMatchState(match) {
        ...
        if (firstScorePending && secondScorePending) {
            return {
                status: "pending",
                winnerIndexes: [],
            };
        ...
        if (Number.isNaN(firstScore) || Number.isNaN(secondScore)) {
            return {
    ...
    function createAdvancedPlayer(winner, currentTargetPlayer = {}, previousMatchId) {
        ...
        const isSamePlayer =
            winnerIdentity &&
            currentIdentity &&
            winnerIdentity === currentIdentity;
    ...
    function propagateWinner(roundIndex, matchIndex) {
        ...
        const winner =
            matchState.status === "completed"
                ? match.players[matchState.winnerIndexes[0]]
                : null;

        if (!winner) {
            nextMatch.players[playerIndex] = createEmptyPlayer();
        } else {
            nextMatch.players[playerIndex] = createAdvancedPlayer(
                winner,
                nextMatch.players[playerIndex],
                match.matchId
            );
        }
    }
    ...
    function createBracketLayout(bracket) {
        ...
        for (let roundIndex = 1; roundIndex < rounds.length; roundIndex++) {
            ...
            currentRound.matches = currentRound.matches.map((match, matchIndex) => {
                ...
                const firstCenterY = getMatchCenterY(firstSourceMatch);
                const secondCenterY = getMatchCenterY(secondSourceMatch);
                const y = ((firstCenterY + secondCenterY) / 2) - (matchHeight / 2);
    ...
    function calculateLayoutHeight(rounds) {
        return Math.max(
            ...rounds.flatMap((round) =>
                round.matches.map((match) => match.y + match.height)
            )
        );
    }
    ...
    function createMatchElement(match) {
        ...
        element.addEventListener("click", () => {
            if (typeof config.onMatchClick === "function") {
    ...
    function init() {
        ...
        const layout = createBracketLayout(bracketState);

        renderBracket(container, layout);
    }
    ...
    function updateMatch(matchId, nextMatchData) {
        ...
        if (updatedRoundIndex === -1 || updatedMatchIndex === -1) {
        ...
        if (typeof config.onMatchUpdate === "function" && updatedMatch) {
    ...
    function getState() {
        if (typeof structuredClone === "function") {
            return structuredClone(bracketState);
        }
